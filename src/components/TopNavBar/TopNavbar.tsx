  ...
  const handleKeyDown = (e: React.KeyboardEvent, index: number) => {
    switch (e.key) {
      ...
      case 'ArrowDown':
        ...
        setSelectedListItem(nextIndex);
        document.getElementById(`list-item-${nextIndex}`)?.focus();
        break;
      case 'ArrowLeft':
      case 'ArrowUp':
        e.preventDefault();
        const prevIndex = (index - 1 + navItemsList.length) % navItemsList.length;
        setSelectedListItem(prevIndex);
        document.getElementById(`list-item-${prevIndex}`)?.focus();
        break;
    }
  };
